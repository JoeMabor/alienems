...
class ManageEmployeeUseCase(ManageEmployeeUseCasePort):
    ...
    def create_employee(self, employee_entity: EmployeeEntity, work_arrangement: WorkArrangementEntity = None):
        ...
        if employee_entity.is_part_time():
            ...
            return self._add_part_time_employee(employee_entity=employee_entity, work_arrangement=work_arrangement)
        else:
            return self._add_full_time_employee(employee_entity)

    def delete_employee(self, employee_pk: int):
        if self._employee_repo.employee_exists(employee_pk):
            # delete
            self._employee_repo.delete(employee_pk)
        else:
